...
async function loadDashboard(userId) {
    try {
        ...
        if (data.lastSession) {
            continueText.textContent = `You were solving ${data.lastSession.subject} – ${data.lastSession.paper}.`;
            continueBtn.onclick = () => {
...
async function loadTopScholars() {
    try {
        ...
        // Clear existing dummy rank items
        sidebar.querySelectorAll('.rank-item').forEach(el => el.remove());
        ...
        snapshot.forEach((doc, index) => {
            ...
            div.innerHTML = `<span>${medal} ${data.name}</span><b>${data.points} pts</b>`;
